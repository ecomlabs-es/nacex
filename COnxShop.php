...
if (!Validate::isPostCode($cp) || empty($cp)) {
...
?>
    ...
    <h1 id="cabecerashop" style="color:orange;">Nacexshop</h1>
    </body>
...
$_coordenadas = $_ws->get_Agencia3($cp);
...
if (Configuration::get('NACEX_GOOGLE_API') != '' && is_null($bname)) {
    echo $_nacex_shop->nacex_shop_map($_agencias, $cp, $conn);
} else {
    echo $_nacex_shop->nacex_shop_list($_agencias, $cp, $conn);
}
?>
    </html>
